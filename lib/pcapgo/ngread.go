...
// readBlock reads the blocktype and length from the file. If the type is a section header, endianness is also read.
func (r *NgReader) readBlock() error {
	...
	if r.currentBlock.typ == ngBlockTypeSectionHeader {
		...
		if binary.BigEndian.Uint32(r.buf[8:12]) == ngByteOrderMagic {
			r.bigEndian = true
			r.microsecondTime = true
		} else if binary.LittleEndian.Uint32(r.buf[8:12]) == ngByteOrderMagic {
			r.bigEndian = false
			r.microsecondTime = true
		} else if binary.BigEndian.Uint32(r.buf[8:12]) == ngByteOrderMagicNS {
			r.bigEndian = true
			r.microsecondTime = false
		} else if binary.LittleEndian.Uint32(r.buf[8:12]) == ngByteOrderMagicNS {
			r.bigEndian = false
			r.microsecondTime = false
		} else {
